...
rules = [
    ...
    ('<main_function>', ['<type_specifier>', '<main_function_identifier>', '<parameter_list>',  '<compound_statement>']),
    ...
    ('<main_function_identifier>', ['main_f']),
...
def parse(tokens, rule, kleene_dict=None):
    ...
    print(rule[0])
    if len(rule) > 2:
        called_production_rule = rule[2]
        subrules = [r for r in rules if r[0] == called_production_rule]
        if not subrules:
            raise ValueError("Invalid production rule: " + called_production_rule)
        match_found = False
        ...
        if not match_found:
            raise ValueError("No matching subrule found for production rule: ", called_production_rule)
