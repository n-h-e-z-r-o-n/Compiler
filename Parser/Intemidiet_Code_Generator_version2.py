...
def serach(my_dict, target_value):  # function that is used to search for same information in our disct dictionary
    for key, value in my_dict.items():
        if value[1] == target_value:  # why value[1] and not value[0]?
            return key  # why? because in a dict we can have key-value of t1 = a; t1 is key while a is value. when we return key, we are returning the address of that value
    return target_value
...
count = 1  # keep track of temporary variables in the Intimidate_Code, t1, t2,...
label_track = 1  # keep track of Label variables in the Intimidate_Code that are used for ~ goto ~ e.g goto L1, L2,...
        ...
        if node_name == "DECLARATION":
            ...
            for child in children:
                if child[0] == 'condition':
                    exp = []  # temporary list for storing comparison expression
                    for x in child[1]:
                    ...
                    l = label_track
                    t_V = t_V_prev = f"t{count}"  # count of the current temporary variable

                    try: # check which operator/comparator is in the if condition statement and store this in a temp variable
                            if exp[1] == '==':
                                print(f"(EQ, {t_V}, {exp[0]}, {exp[2]})")
                            elif exp[1] == '!=':
                                print(f"(NEQ, {t_V}, {exp[0]}, {exp[2]})")
                            elif exp[1] == '>':
                                print(f"(GT, {t_V}, {exp[0]}, {exp[2]})")
                            elif exp[1] == '<':
                                print(f"(LT, {t_V}, {exp[0]}, {exp[2]})")
                            elif exp[1] == '>=':
                                print(f"(GEQ, {t_V}, {exp[0]}, {exp[2]})")
                            elif exp[1] == '<=':
                        ...
                        count += 1
                        t_V = f"t{count}"
                        print(f"(NOT, {t_V}, {exp[0]})")

                    count += 1
                    print(f"(IF, {t_V}, L{l})")  # Check NOT condition in the if and jump to the else label if true
            ...
            for child in children[0]:
                if child[0] == 'if_condition':
                    ...
                    label_track += 1
                    t_V = t_V_prev = f"t{count}"  # count of the current temporary variable
                    # check which operator/comparator is in the if condition statement and store this in a temp variable
                    try:  # check which operator/comparator is in the if condition statement and store this in a temp variable
                        if exp[1] == '==':
                            print(f"(EQ, {t_V}, {exp[0]}, {exp[2]})")
                        elif exp[1] == '!=':
                    ...
                    print(f"(IF, {t_V}, L{label_track})")  # Check NOT condition in the if and jump to the else label if true
...
start_run_time_time_ICG = time.time()  # Record the Start run time-time of Intimidate_Code_Generator
Intemidiet_Code_Generator(parser_tree)  # calling the Intimidate_Code_Generator function and passing parser_tree-list
End_run_time_time_ICG = time.time()  # Record the End run time-time of Intimidate_Code_Generator
Program_Run_time_ICG = End_run_time_time_ICG - start_run_time_time_ICG  # Calculate the elapsed time (run time of Intimidate_Code_Generator function)
print(f"\nIntimidate_Code_Generator Program Runtime  :  {Program_Run_time_ICG} seconds")
